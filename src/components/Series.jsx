...
// ── Desktop card: full sticky scale+rotate scroll effect ──
const SeriesCardDesktop = ({ card, index }) => {
  const vertMargin = 10;
  ...
  // Clamp the counter-rotation to ±6 degrees max to avoid GPU overload
  ...
  scrollY.on('change', (y) => {
    ...
    // Clamp filter to 0–6 degrees max (was 0–100, too heavy for mobile GPUs)
...
// ── Mobile card: simple fade-in, no sticky/scale/rotate ──
const SeriesCardMobile = ({ card, index }) => {
  ...
  return (
    <motion.div
      ...
      className="w-full max-w-lg mx-auto rounded-[24px] overflow-hidden bg-white shadow-[0px_4px_24px_rgba(0,0,0,0.18)] border border-stone-200/60"
    >
      <CutoutCard
        trackPointerHover={false}
        className="group/cutout relative cursor-pointer overflow-hidden rounded-[24px] bg-white text-stone-900"
      >
        {/* ── Media ── */}
        <CutoutCardMedia className="h-[56vw] max-h-[280px]">
          <img
            src={card.image}
            srcSet={`${card.image.replace('w=800', 'w=400')} 400w, ${card.image} 800w`}
            sizes="100vw"
          ...
          {/* Tag label — bottom-left */}
          <CutoutCardInsetLabel className="bottom-0 left-0 rounded-tr-[20px] bg-white px-4 py-2">
            <span className="text-[9px] font-bold uppercase tracking-[3px] text-stone-500">
              {card.tag}
            </span>
            <CutoutCorner className="absolute -right-[31px] -bottom-px rotate-90 text-white" size={32} />
            <CutoutCorner className="absolute -top-[31px] -left-px rotate-90 text-white" size={32} />
          ...
          {/* Series number pin — top-right */}
          <CutoutCardPin className="top-0 right-0 rounded-bl-[20px] bg-white px-4 py-2">
            <span className="text-[9px] font-bold uppercase tracking-[3px] text-stone-600">
              0{index + 1}
            </span>
            <CutoutCorner className="absolute -left-[31px] -top-px rotate-[270deg] text-white" size={32} />
            <CutoutCorner className="absolute -bottom-[31px] -right-px rotate-[270deg] text-white" size={32} />
        ...
        <CutoutCardContent className="px-5 pt-5 pb-5">
          ...
          >
            <motion.h3
              variants={stagger.item}
              className="font-serif text-xl font-semibold text-stone-900 mb-2 tracking-tight leading-snug"
            >
              {card.title}
            </motion.h3>
            <motion.p
              variants={stagger.item}
              className="text-stone-500 text-xs font-light leading-relaxed mb-5"
            >
              {card.description}
            </motion.p>
            <motion.div variants={stagger.item} className="w-full h-px bg-stone-200 mb-4" />
            <motion.div variants={stagger.item} className="flex items-center justify-between">
              ...
              <button className="rounded-full border border-stone-300 bg-stone-50 active:bg-stone-100 px-4 py-1.5 text-[11px] font-semibold tracking-[1.5px] uppercase text-stone-600">
                Enquire
              </button>
            </motion.div>
          </motion.div>
...
const Series = () => {
  ...
  return (
    // FIX 1: Removed ReactLenis wrapper — it conflicts with native touch-scroll on real mobile devices.
    // FIX 2: Removed `bg-fixed` from className — background-attachment:fixed is broken on iOS/Android.
    <section
      id="series"
    ...
    >
      ...
      {/* FIX 3 & 4: Render simple mobile cards or full sticky-scroll desktop cards based on device */}
      {isMobile ? (
        <div className="max-w-lg mx-auto flex flex-col gap-8 pb-16">
          {seriesCards.map((card, i) => (
            <SeriesCardMobile key={card.id} card={card} index={i} />
          ))}
        </div>
      ) : (
        <div className="max-w-2xl mx-auto flex flex-col gap-[30vh] pb-[50vh]">
          {seriesCards.map((card, i) => (
            <SeriesCardDesktop key={card.id} card={card} index={i} />
          ))}
        </div>
      )}
